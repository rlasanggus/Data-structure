...
void BST_hight_scan(BST_TREE* tree){
	T_NODE* search_root = tree -> root;

	int hight = 0;
	T_NODE* small = find_smallest_node(search_root, &hight);
	printf("smallest_node hight(left hight) : %d\n", hight);
	printf("smallest_data : %d\n", small -> data);

	printf("\n");

	hight = 0;
	T_NODE* large = find_largest_node(search_root, &hight);
	printf("largest_node hight(rigt hight) : %d\n", hight);
	printf("largest_node data : %d\n", large -> data);
}
...
T_NODE* find_smallest_node(T_NODE* root, int* hight){
	if(root -> left == NULL)
		...
		(*hight)++;
		return find_smallest_node(root -> left, hight);
	}
}
...
T_NODE* find_largest_node(T_NODE* root, int* hight){
	if(root -> right == NULL)
		...
		(*hight)++;
		return find_largest_node(root -> right, hight);
	}
}